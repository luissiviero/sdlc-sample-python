...
- `sample_pkg/calc.py` adds `percent(part: float, whole: float) -> float` returning `part` as a
  percentage of `whole`; `percent(1, 4)` returns `25.0` (intent Proposed outcome).
...
- Security-baseline: does not fully apply — `percent` is pure arithmetic on its two arguments;
  ...
  `risk_list` (auth, data migrations, money movement, production config): none of those are
  touched.
...
- `sample_pkg/calc.py`: add `percent(part: float, whole: float) -> float` after `divide`,
  implemented as `return divide(part, whole) * 100`. Reusing `divide` gives the zero-check one
  implementation (see Open questions below) and keeps the file's existing two-function,
  no-docstring style.
...
- "Should `percent` reuse `divide` internally so the zero-total error has one source, or check
  for zero itself?" — Answered: reuse `divide` (`percent = divide(part, whole) * 100`). This
...
- `tests/test_calc.py` passes two new cases: a normal case equivalent to `percent(1, 4) ==
  25.0`, and `pytest.raises(ZeroDivisionError)` for a zero `whole`.
