...
- `sample_pkg/calc.py` — add `percent(part: float, whole: float) -> float`, defined after
  `divide`, implemented as `return divide(part * 100, whole)`.
- `sample_pkg/__init__.py` — import `percent` from `.calc` alongside `add` and `divide`; add
  `"percent"` to `__all__` after `"divide"`.
- `tests/test_calc.py` — add `test_percent` (asserts `percent(1, 4) == 25.0` and the
  exact-percentage input `percent(7, 100) == 7.0`) and `test_percent_zero_whole_raises`
  (`pytest.raises(ZeroDivisionError): percent(1, 0)`),
...
3. In `tests/test_calc.py`, add `test_percent` (two plain asserts: `percent(1, 4) == 25.0`
   and `percent(7, 100) == 7.0`) and `test_percent_zero_whole_raises` after the existing two
   tests, importing `percent` alongside `add` and `divide` in the existing
...
- Operand order: step 1 scales `part` before calling `divide`. The other order,
  `divide(part, whole) * 100`, rounds the quotient first and returns `7.000000000000001` for
  `percent(7, 100)` (also `28.999999999999996` for 29/100, `56.99999999999999` for 57/100),
  while the planned `percent(1, 4) == 25.0` assertion cannot tell the two apart because 0.25 is
  exactly representable. Mitigation: `test_percent` also asserts `percent(7, 100) == 7.0`, which
  fails under the other order, so the build cannot ship it green.
- No new dependency and none of `sdlc.yaml`'s `risk_list` items apply, so this change does not
  park on that check.
...
- `divide(part, whole) * 100` (multiply after dividing): the formula of the first draft,
  replaced after the round-1 adversarial review showed it returns an inexact value for
  exact-percentage inputs (see Risks). `divide(part * 100, whole)` keeps every other decision:
  delegation to `divide`, one zero-check, the same `ZeroDivisionError` and message, raw float,
  no rounding.
...
## Proof
- `tests/test_calc.py::test_percent` — new test, written in step 3 — asserts
  `percent(1, 4) == 25.0` and `percent(7, 100) == 7.0`.
